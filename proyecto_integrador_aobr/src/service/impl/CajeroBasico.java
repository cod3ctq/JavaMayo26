...
public class CajeroBasico extends Atm implements IOperacionesBasicas {
    ...
    public Ticket cobrarRetiroSinTarjeta() throws InvalidWithdrawalWithoutCardException, CardlessWithdrawalAlreadyProcessedException {
        ...
        if(!existe){
            ...
            if(seleccion !=1){
                ...
                //construye el objeto aplicando el patron builder: encadena solo los datos que tengas, asi sean todos o solos algunos
    ...
    public Object[] retirar(String numTarjeta, double monto, String nip) throws MaxDailyWithdrawalsExceededException, InvalidQuantityException, InsufficientBalanceException, UnderMinimunException{
        ...
        try{
            ...
            //Verificar si me alcanza
                ...
                Ticket t = Ticket.builder().direccion(this.getDireccion()).folio(folioOperacion++).fecha(LocalDateTime.now()).monto(monto).tipoOperacion("RETIRO").cuenta("*******"+cuenta.getNumCuenta().substring(8)).build();
                datos[0]=monto;
                datos[1]=t;
            }
    ...
    public Ticket pagarServicio(String numTarjeta,String convenio, String referencia) {
        ...
        if(ok){
            ...
            ticket= Ticket.builder().direccion(this.getDireccion()).folio(folioOperacion++).fecha(LocalDateTime.now()).tipoOperacion("PAGO SERVICIOS").build();
